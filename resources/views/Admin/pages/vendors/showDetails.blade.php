...
@section('content')
    <div class="content-wrapper">
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-3 mb-4 text-center"><span class="text-muted fw-light">
                </span> {{ __('messages.Vendor') }}
            </h4>
            ...
            <div class="card">
                <div class="card-body">
                        ...
                        <table class="table table-striped text-black text-center">
                            <tbody class="text-center">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form action="{{ route('vendors.updateStatus', $vendor->id) }}" method="POST"
                                            class="d-inline">
                                            @csrf
                                            <input type="hidden" name="status" value="{{ $vendor->status ? 0 : 1 }}">
                                            <button type="submit"
                                                class="btn btn-sm {{ $vendor->status ? 'btn-danger' : 'btn-success' }}">
                    ...
                    <a href="{{ route('vendore.index') }}" class="btn btn-secondary mt-3">{{ __('messages.Back') }}</a>
                </div>
            </div>
        </div>
    </div>
